...
public class ExcelUtils {
   ...
   * 初始化Excel
   */
  public static void initExcel(File file, String[] colName) throws Exception {
    format();
  ...
  public static <T> void writeObjListToExcel(List<T> objList, File file, Context c)
      throws Exception {
    if (objList != null && objList.size() > 0) {
      ...
      try {
        ...
        writebook.write();
        ToastUtils.showShort("导出到手机存储中文件夹Record成功");
      } finally {
        if (writebook != null) {
          try {
            writebook.close();
