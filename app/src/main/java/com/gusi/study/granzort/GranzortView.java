...
public class GranzortView extends View {
  ...
  //三个阶段的枚举
  private enum State {
    CIRCLE_STATE, TRANGLE_STATE, FINISH_STATE
  }
  ...
  @Override protected void onDraw(Canvas canvas) {
    super.onDraw(canvas);
    canvas.drawColor(getResources().getColor(R.color.other));
    canvas.save();
    canvas.translate(mViewWidth / 2, mViewHeight / 2);
    float stopD1 = distance * pathMeasure.getLength();
    switch (mCurrentState) {
      case CIRCLE_STATE:
        drawPath.reset();
        pathMeasure.setPath(innerCircle, false);
        //不断截取起点到*%长度的 path 赋值给drawPath
        pathMeasure.getSegment(0, stopD1, drawPath, true);
  ...
  private void initAnimatorListener() {
    ...
    animatorListener = new Animator.AnimatorListener() {
      @Override public void onAnimationStart(Animator animation) {
        Logger.w("star:", mCurrentState + "_");
      }

      @Override public void onAnimationEnd(Animator animation) {
        Logger.w("end:", mCurrentState + "_");
  ...
  private void initPath() {
    ...
    Logger.w(pos[0] + ":--:" + pos[1] + ":--:" + pathMeasure.getLength() * 2 / 3);
