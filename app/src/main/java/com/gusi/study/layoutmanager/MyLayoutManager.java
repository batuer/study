package com.gusi.study.layoutmanager;
...
public class MyLayoutManager extends RecyclerView.LayoutManager {
  ...
  private void recycleAndFillItems(RecyclerView.Recycler recycler, RecyclerView.State state) {
    ...
    for (int i = 0; i < getItemCount(); i++) {
      ...
      if (Rect.intersects(displayFrame, allItemFrames.get(i))) {
        ...
        Rect frame = allItemFrames.get(i);
        //将这个item布局出来
        layoutDecorated(scrap, frame.left, frame.top - verticalScrollOffset, frame.right,
  ...
  public int scrollVerticallyBy(int dy, RecyclerView.Recycler recycler, RecyclerView.State state) {
    ...
    recycleAndFillItems(recycler, state);
    Log.w(TAG, "scrollVerticallyBy: " + getChildCount());
    return travel;
  }
