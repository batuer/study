...
public class FadingTextView extends android.support.v7.widget.AppCompatTextView {
  ...
  public static final int MILLISECONDS = 1, SECONDS = 2, MINUTES = 3;
   ...
   * Should only be used if you notice {@link #onAttachedToWindow()} ()} is not being executed as
   * expected
   */
  public void resume() {
    isShown = true;
   ...
   * Should only be used if you notice {@link #onDetachedFromWindow()} is not being executed as
   * expected
   */
  public void pause() {
    isShown = false;
  ...
  /**
   * Stops the animation
   * Unlike the pause function, the stop method will permanently stop the animation until the view
   * is restarted
   */
  public void stop() {
    isShown = false;
  ...
  private void handleAttrs(AttributeSet attrs) {
    ...
    this.texts = a.getTextArray(R.styleable.fading_text_view_texts1);
    this.timeout = Math.abs(a.getInteger(R.styleable.fading_text_view_timeout, 14500))
  ...
  // * @deprecated use {@link #setTimeout(long, java.util.concurrent.TimeUnit)} instead.
  // */
  //@Deprecated public void setTimeout(double timeout, @TimeUnit int timeUnit) {
  //  if (timeout <= 0) {
  //    throw new IllegalArgumentException("Timeout must be longer than 0");
  //  } else {
  //    int multiplier;
  ...
  @Override public void startAnimation(Animation animation) {
    if (isShown && !stopped) super.startAnimation(animation);
  }
  ...
  @IntDef({ MILLISECONDS, SECONDS, MINUTES }) @Retention(RetentionPolicy.SOURCE)
